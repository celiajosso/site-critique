...
function articlesOnPrivatePage($my_sqli, $id_connected) {
    ...
    $sql_data_article = "SELECT id_Article, titre_Article, noteRedacteur_Article, dateCreation_Article FROM Article INNER JOIN Utilisateur ON Article.id_UtilisateurCreateur = Utilisateur.id_Utilisateur WHERE id_Utilisateur=$id_connected";
    $sql_data_article_res = readDB($my_sqli, $sql_data_article);

    $sql_data_jaquette = "SELECT chemin_Image FROM Image INNER JOIN est_Image ON est_Image.id_Image = Image.id_Image INNER JOIN Article ON Article.id_Article = est_Image.id_Article WHERE Article.id_UtilisateurCreateur=$id_connected AND chemin_Image LIKE '%jaquette%'";
    $sql_data_jaquette_res = readDB($my_sqli, $sql_data_jaquette);

    $sql_data_note = "SELECT AVG(note_Avis) as note_moy FROM Avis INNER JOIN Article ON Article.id_Article = Avis.id_Article WHERE Article.id_UtilisateurCreateur=$id_connected";
    $sql_data_note_res = readDB($my_sqli, $sql_data_note);
